...
class Bank:
    ...
    def __init__(self, name, unit, value = 0):
        self.name = name
        self.unit = unit # Currency class
        self.current_account = unit(value) # Currency Instance
    ...
    def __str__(self):
        ...
        return "{} Bank holds the {} currency and currently holds {} of {}".format(self.name, unit_string, self.current_account.value, unit_string)

    def deposit(self, currency_instance):
        if isinstance(currency_instance, self.unit): # Can also use self.unit == type(currency_instance)
            # Add value of deposit to bank's value
